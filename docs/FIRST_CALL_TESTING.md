...
1. 覆盖安装 `OrderEcho-first-call-D4.apk`，设置中保持「开启来电识别」，并确认系统没有关闭本应用通知；常驻「来电识别已开启」通知仍是监听已开启的标志。
2. 删除一个测试号码的全部系统通话记录，按电源键锁屏后用该号码打入，保持响铃约 10 秒。锁屏来电界面应出现系统的「第一次来电」提示通知；不显示号码，也不增加铃声或震动。
3. 接听、挂断或等待提示超时后，该临时提示通知应自动消失。保留该号码的来电记录后再次打入，不应出现首次提示。
4. 诊断页出现「锁屏：已发出系统首次来电提示」表示系统通知已请求；若未看到提示，请同时检查 App 的通知是否被系统关闭并发送诊断照片。
