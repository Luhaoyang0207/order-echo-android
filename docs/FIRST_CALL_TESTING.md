...
1. 覆盖安装 `OrderEcho-first-call-D8.apk`，保持 D7 已设置的「首次来电识别（锁屏提示）」高优先级和静音设置。
2. 清空来电诊断，删除一个测试号码的全部系统通话记录，锁屏后用该号码打入。
3. 「第一次来电」应只在顶部显示。号码、接听、挂断、短信和提醒等华为原生来电操作必须仍清楚可见且可直接使用；无需点击「返回来电」。
4. 接听、挂断或等待最多 12 秒后提示自动关闭。解锁状态仍使用原有悬浮窗。
5. 若仍遮挡任何原生操作，请连续拍摄提示出现时和点击接听／挂断后的照片，并发送诊断截图。

## D9：锁屏通知横幅，不切走来电界面

1. 覆盖安装 `OrderEcho-first-call-D9.apk`。在「设置 → 打开锁屏提示通知设置」中，继续使用 `首次来电识别（锁屏提示）` 这个渠道，保持高优先级／横幅和锁屏显示开启，声音与振动关闭。
2. 清空来电诊断，删除一个测试号码的全部系统通话记录，然后锁屏并用该号码拨入。
3. 响铃期间，华为原生来电画面必须持续可见，号码、接听、挂断、短信和提醒操作必须直接可用，且不应出现「点击返回来电」。若 EMUI 允许该静音高优先级渠道在来电界面显示，屏幕顶部会短暂出现只含「第一次来电」的横幅。
4. 接听、挂断或 12 秒后横幅必须自动消失。解锁来电仍使用原有的顶部悬浮窗。
5. 若诊断显示「锁屏：已发出系统首次来电提示」和「锁屏提示：通知渠道允许横幅提示」，但来电界面没有横幅，请发送来电照片和诊断页。D9 不会启动 Activity；这种结果说明 EMUI 隐藏了普通静音通知横幅，而不是应用覆盖或切走了来电界面。
## D10：无障碍锁屏顶部提示

1. 安装 `OrderEcho-first-call-D10.apk`，打开「设置」，然后点击「打开无障碍设置」。电话、通话记录和普通悬浮窗权限仍须保持已允许。
